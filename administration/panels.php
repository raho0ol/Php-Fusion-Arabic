    ...
    private function panelReactor($side) {
        ...
        if (isset($this->panel_data[$side])) {
            foreach ($this->panel_data[$side] as $data) {
                ...
                $html .= "<li class='dropdown-item'><a href='".FUSION_SELF.$aidlink."&action=delete&panel_id=".$data['panel_id']."' onclick=\"return confirm('".self::$locale['PANEL_440']."');\"><i class='fa fa-trash m-l-10 m-t-5'></i>".self::$locale['delete']."</a>\n</li>\n";
                $html .= "</ul>\n";
                $html .= "</div>\n";
                $html .= "</div>\n";
                $html .= "</li>\n";
